...
// Opt this route into static generation with ISR (revalidate) and retain tag-based invalidation
export const dynamic = 'force-static';
export const revalidate = 3600; // Revalidate every hour

const getLatestChapters = async (time: Date) => {
  return await db.query.chapter.findMany({
    where: (chapter, { gte, and, eq }) =>
      and(gte(chapter.publishedAt, time), eq(chapter.premium, false)),
    columns: {
      slug: true,
      publishedAt: true,
      number: true,
  ...
  for (const chapter of chapters) {
    feed.item({
      title: `${chapter.novel.title} ${
        chapter.volume.number > 0 ? `Volume ${chapter.volume.number} ` : ""
